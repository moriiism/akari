#
# add_flag_pca_after_lrm.py
#
# add pca components (after left-right match)
#
...
# python3 add_flag_pca_after_lrm.py 1 0 0 def def def def
...
if (7 == nargs):
    ...
    print('usage: python3 add_flag_pca_after_lrm.py ' +
...
# tag
pca_tag_str = ("pca_cat%d_ftr%d_prefit%d_20240530_after_lrm" %
               (flag_cat, pca_feature, use_prefit))
...
# pca and gmm calc after selection of left-right match: "star_pos" > 1
...
if (use_prefit == 0):
    ...
    sc = StandardScaler()
    data_selrow_selcol_sc_ndarr = sc.fit_transform(data_selrow_selcol_df)
    ...
    out_sc_pkl = outdir + "/" + "sc.pkl"
    out_pca_pkl = outdir + "/" + "pca.pkl"
    pk.dump(sc, open(out_sc_pkl,"wb"))
    pk.dump(pca, open(out_pca_pkl,"wb"))
else:
...
data_selcol_sc_ndarr = sc.transform(data_selcol_df)
